...
example {a b : ℚ} (h1 : 3 ≤ a) (h2 : a + 2 * b ≥ 4) : a + b ≥ 3 :=
  calc a + b = 1/2*((a + 2 * b)+a) := by ring
    _≥ 1/2*(4+3) := by rel[h1,h2]
    _≥ 3 := by numbers

example {x : ℤ} (hx : x ≥ 9) : x ^ 3 - 8 * x ^ 2 + 2 * x ≥ 3 :=
  calc x ^ 3 - 8 * x ^ 2 + 2 * x = x * ( x^2 - 8 * x + 16 + 2 -16) := by ring
...
calc n ^ 4 - 2 * n ^ 2 = n ^ 4 - 2 * n ^ 2 - 3 * n ^ 3 + 3 * n ^ 3 := by ring
  ...
  _= n ^ 2 * (n ^ 2 - 4 * n + 4 + n -6 ) + 3 * n ^ 3 := by ring
  _= n ^ 2 * ((n-2)^2 + n -6 ) + 3 * n ^ 3 := by ring
  _= n ^ 2 * (n-2)^2 + n ^ 2 * (n -6) + 3 * n ^ 3 := by ring
  _≥ 10 ^ 2 * (10-2)^2 + 10 ^ 2 * (10 -6) + 3 * n ^ 3 := by rel [hn]
  _> 3 * n ^ 3 := by extra

example {n : ℤ} (h1 : n ≥ 5) : n ^ 2 - 2 * n + 3 > 14 :=
  calc n ^ 2 - 2 * n + 3 = n ^ 2 - 2 * n + 1 + 2 := by ring
  _= ( n - 1 ) ^ 2 + 2 := by ring
  _≥ (5-1)^2 + 2:= by rel [h1]
  _=18 := by numbers
  _>14 := by numbers
...
example {x : ℚ} : x ^ 2 - 2 * x ≥ -1 :=
  calc x ^ 2 - 2 * x = x ^ 2 - 2 * x + 1 - 1 := by ring
  _= (x-1)^2 - 1 := by ring
  _≥ 0 - 1 := by extra
  _≥ -1 := by numbers

example (a b : ℝ) : a ^ 2 + b ^ 2 ≥ 2 * a * b :=
  calc a ^ 2 + b ^ 2 = a ^ 2 + b ^ 2 - 2 * a * b + 2 * a * b := by ring
